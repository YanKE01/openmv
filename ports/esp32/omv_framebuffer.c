...
void framebuffer_init(framebuffer_t *fb, void *buff, size_t size, bool dynamic, bool enabled) {
    ...
    fb->dynamic = dynamic;
    fb->enabled = enabled;
    fb->quality = OMV_JPEG_QUALITY_LOW;
    mutex_init0(&fb->lock);
}
...
void framebuffer_init0() {
    ...
    if (stream_fb->raw_base != NULL) {
        stream_enabled = stream_fb->enabled;
    }
    ...
    framebuffer_init(stream_fb, fb_stream_mem, OMV_SB_SIZE, true, stream_enabled);
    memset(stream_fb->raw_base, 0, omv_esp32_stream_header_size);
}
...
void framebuffer_update_preview(image_t *src) {
    ...
    if ((src == NULL) || (fb == NULL) || !fb->enabled) {
        return;
    }
    ...
    encode_cfg.image_quality = fb->quality ? fb->quality : OMV_JPEG_QUALITY_LOW;
